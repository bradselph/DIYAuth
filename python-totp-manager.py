...
INTERNAL_STORAGE_FILE = os.path.join(base_path, "internal_storage.dat")
...
class TOTPManager(QMainWindow):
    def __init__(self):
        ...
        self.dump_accounts_file()  # Add this line to dump the contents of accounts.json
    ...
    def load_config(self):
        try:
            config = json.loads(self.get_embedded_data('config'))
    ...
    def save_config(self):
        ...
        self.set_embedded_data('config', json.dumps(config))
...
def dump_accounts_file(self):
    ...
    def get_embedded_data(self, name):
        try:
    ...
    def set_embedded_data(self, name, data):
        try:
            storage = {}
            try:
                with open(INTERNAL_STORAGE_FILE, 'rb') as f:
    ...
    def load_accounts(self):
        ...
        self.logger.debug(f"Encrypted data retrieved: {encrypted_data[:20]}...") # Log first 20 chars
    ...
    def verify_account_data(self, accounts):
        # Implement a simple integrity check
        return all('name' in account and 'secret' in account for account in accounts)
